...
#SBATCH -J vlm3r_pi3x_scannetpp_extract
...
python $REPO/scripts/extract_spatial_features_pi3x.py \
  --input-dir $DATA_ROOT/scannetpp/videos \
  --output-dir $DATA_ROOT/scannetpp/spatial_features_pi3x \
  --pi3x-weights-path /leonardo_scratch/fast/EUHPC_D32_006/hf_models/Pi3X \
  --processor-config-path $REPO/processor_config.json \
  --filter-by-jsons \
    $DATA_ROOT/VLM-3R-DATA/vsibench_train/merged_qa_scannetpp_train.json \
  --gpu-ids 0 \
  --batch-size 1 \
  --frames-upbound 32 \
  --precision bf16
