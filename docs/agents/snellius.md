...
Snellius currently has two deliberately separate workflows:

- The migrated SpatialStack SFT/evaluation smoke pipeline uses the checkout at
  `/home/geusdd/VLM-3R`, the scoped paths in `snellius/paths.env`, and the
  wrappers documented in `snellius/README.md`.
- The formal five-candidate pre-SFT proxy migration is governed by
  `docs/snellius_proxy_migration_manifest.md` and
  `docs/snellius_proxy_migration_files.txt`.

Do not exchange checkpoints, caches, paths, environments, or conclusions
between these workflows merely because both run on Snellius. In particular,
the SFT smoke checkpoint is forbidden input to the formal pre-SFT proxy.
...
- The migrated SFT/evaluation smoke wrappers use account-specific resources
  recorded in `snellius/README.md`; account `tesei2748` exposed only normal
  QoS at validation time. Do not copy Leonardo's `boost_qos_dbg` setting or
  assume the proxy partition is the correct SFT partition.
...
## Active SFT/Evaluation Layout

For the validated SFT/evaluation smoke pipeline, source:

    source /home/geusdd/VLM-3R/snellius/paths.env

The scoped file defines the active checkout, model, data, sidecar, checkpoint,
cache, log, and migration roots beneath `/home/geusdd/VLM-3R` and
`/scratch-shared/geusdd/VLM3R`. Do not edit shared Leonardo wrappers merely to
run them on Snellius. Use the dedicated `snellius/*.sbatch` entry points and
validators listed in `snellius/README.md`.

Shared scratch is purgeable. It is suitable for the currently validated
bundle but is not the sole durable location for an official long campaign.
Do not use inode-exhausted project space or `/scratch-local` for persistent
artifacts.

The validated environment is described in `snellius/environment-status.md`.
Use the absolute environment `bin` path as the wrappers do; loading the
Miniconda module and then activating by environment name can select the wrong
Python. Never copy compiled Leonardo extensions to Snellius.

## Formal Pre-SFT Proxy Intended Layout

The following `/home/shuang` layout belongs specifically to the formal proxy
migration audit. The repository is code only. Do not put datasets, models, C1 artifacts, proxy
...
At the formal-proxy inspection, only /home/shuang/SpatialFocus existed; the five intended
...
Use the existing environment for the selected workflow. For the formal proxy
audit, modules, Conda base, and Conda vsibench were visible on the inspected
login node; vlm3r was not. Do not recreate,
