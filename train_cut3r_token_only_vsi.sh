...
# ============================================================
SUFFIX="${SUFFIX:-cut3r_token_only_vsi_lora}"
...
cleanup_on_training_failure() {
    ...
    if [[ "$status" -ne 0 ]]; then
        echo "[ERROR] Official training wrapper failed with status $status; it will not cancel a non-SMOKE allocation automatically."
    fi
    exit "$status"
}
