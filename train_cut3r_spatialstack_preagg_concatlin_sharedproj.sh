#!/bin/bash
#SBATCH --job-name=cut3r_ss_preagg_concat
#SBATCH --nodes=4
...
export SUFFIX="${SUFFIX:-cut3r_spatialstack_preagg_concatlin_sharedproj_dec${SANITIZED_PREAGG_LAYERS}_llm${SANITIZED_LLM_LAYERS}}"
export TRAIN_RUN_NAME="${TRAIN_RUN_NAME:-${SUFFIX}_${SLURM_JOB_ID:-manual}}"
export NOTE="${NOTE:-CUT3R SpatialStack pre-aggregation: concat linear aggregator, shared token_mlp projector, configurable target LLM layers.}"
