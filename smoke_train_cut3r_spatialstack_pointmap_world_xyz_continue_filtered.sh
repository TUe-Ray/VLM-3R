#!/bin/bash
# Smoke-test the D2 point-map continuation from the last good DeepSpeed checkpoint.
# Uses the filtered train JSONs that remove known Decord EOF-stalling videos.
#SBATCH --job-name=SMOKE_cut3r_spatialstack_d2_pointmap_resume
#SBATCH --nodes=4
...
SOURCE_OUTPUT_DIR="/leonardo_work/EUHPC_D32_006/Train_Model/VLM3R/cut3r_spatialstack_d2_pointmap_45457911"

export NOTE="${NOTE:-SMOKE resume for CUT3R SpatialStack D2 point-map training from checkpoint-800 with filtered bad-video samples.}"
export TRAIN_RUN_NAME="${TRAIN_RUN_NAME:-SMOKE_cut3r_spatialstack_d2_pointmap_resume_${SLURM_JOB_ID:-manual}}"
export OUTPUT_DIR="${OUTPUT_DIR:-/leonardo_work/EUHPC_D32_006/Train_Model/VLM3R/$TRAIN_RUN_NAME}"
export RESUME_MODE="${RESUME_MODE:-continue}"
export RESUME_CHECKPOINT_PATH="${RESUME_CHECKPOINT_PATH:-$SOURCE_OUTPUT_DIR/checkpoint-800}"
...
export MAX_STEPS="${MAX_STEPS:-805}"
export SAVE_STEPS="${SAVE_STEPS:-1000}"
export SAVE_TOTAL_LIMIT="${SAVE_TOTAL_LIMIT:-1}"

echo "==== SMOKE D2 point-map continuation ===="
