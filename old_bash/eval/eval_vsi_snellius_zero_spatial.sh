#!/bin/bash
#SBATCH --job-name=Eval_zero_spatial_features
#SBATCH --nodes=1
...
NOTE="Evaluation of zero_spatial_features checkpoint using Snellius-specific offline setup. This script is for parity/ablation checks and is not a general-purpose evaluation script."
# ================================================== User-defined variables ==================================================
echo "-------- Note --------"
echo "  note: $NOTE"
...
NUM_PROCESSES="4"
ZERO_SPATIAL_FEATURES="${ZERO_SPATIAL_FEATURES:-True}"
...
echo "ZERO_SPATIAL_FEATURES=$ZERO_SPATIAL_FEATURES"
echo "=================="
...
if ! grep -q "zero_spatial_features" "$SNELLIUS_REPO_DIR/thinking-in-space/lmms_eval/models/vlm_3r.py"; then
  echo "[ERROR] This Snellius lmms_eval does not support zero_spatial_features model arg."
  echo "        Please sync thinking-in-space/lmms_eval/models/vlm_3r.py from VLM-3R first."
  exit 1
fi
...
MODEL_ARGS="pretrained=$PRETRAINED_LOCAL,model_base=$MODEL_BASE_LOCAL,model_name=$MODEL_NAME,conv_template=$CONV_TEMPLATE,max_frames_num=$MAX_FRAMES_NUM,zero_spatial_features=$ZERO_SPATIAL_FEATURES"
accelerate launch \
  ...
  --log_samples \
  --log_samples_suffix "vlm_3r_7b_qwen2_lora_zero_spatial_snellius_parity_offline" \
