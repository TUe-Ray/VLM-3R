#!/bin/bash
#SBATCH --job-name=GeoRoPEIntraPosShuffle_VSiBench
#SBATCH --nodes=1
...
export RUN_NAME="${RUN_NAME:-eval_rope_spherical_100p_40790070_intra_pos_shuffle}"
export OUTPUT_PATH="${OUTPUT_PATH:-$FAST_ROOT/eval/logs/VLM3R/eval_rope_spherical_100p_40790070_intra_pos_shuffle}"
...
export PROBE_INTRA_FRAME_POS_SHUFFLE="${PROBE_INTRA_FRAME_POS_SHUFFLE:-True}"
...
exec > >(tee -a "$OUTPUT_PATH/intra_frame_pos_shuffle_eval.log")

echo "==== Intra-frame position-shuffle VSiBench probe ===="
date
...
summary_cmd=(
  ...
  --kind intra_frame_pos_shuffle
  --learned-dir "$LEARNED_RESULT_DIR"
  --gate0-dir "$GATE0_RESULT_DIR"
  --geometry-shuffle-dir "$GSHUFFLE_RESULT_DIR"
  --intra-frame-pos-shuffle-dir "$OUTPUT_PATH"
  --baseline-dir "$BASELINE_RESULT_DIR"
  --output-dir "$OUTPUT_PATH"
  --primary-label "D2 intra-frame position shuffle"
  --csv-name intra_frame_pos_shuffle_eval_summary.csv
  --md-name intra_frame_pos_shuffle_eval_summary.md
)
...
echo "[DONE] Intra-frame position-shuffle eval artifacts are under: $OUTPUT_PATH"
