#!/bin/bash
#SBATCH --job-name=EvalCUT3RDepthLoss
#SBATCH --nodes=1
...
NOTE="${NOTE:-Leonardo offline VSI-Bench eval for the depth-only CUT3R checkpoint.}"
...
PRETRAINED_LOCAL="${PRETRAINED_LOCAL:-/leonardo_work/EUHPC_D32_006/Train_Model/VLM3R/cut3r_depth_loss_43817021}"
...
OUTPUT_PATH="${OUTPUT_PATH:-$FAST_ROOT/eval/logs/VLM3R}"
...
RUN_NAME="${RUN_NAME:-cut3r_depth_loss_43817021}"
LIMIT="${LIMIT:-0}"
EXTRA_MODEL_ARGS="${EXTRA_MODEL_ARGS:-}"

cd "$REPO_DIR"
mkdir -p logs/eval "$OUTPUT_PATH"
...
for path in "$REPO_DIR" "$SUBMODULE_DIR" "$TASK_DIR" "$PRETRAINED_LOCAL" "$MODEL_BASE_LOCAL" "$SIGLIP_LOCAL" "$SPATIAL_FEATURES_ROOT"; do
  if [[ ! -e "$path" ]]; then
...
for file in \
  "$PRETRAINED_LOCAL/config.json" \
  "$PRETRAINED_LOCAL/adapter_config.json" \
  "$PRETRAINED_LOCAL/non_lora_trainables.bin" \
  "$SIGLIP_LOCAL/config.json" \
  "$TASK_FILE" \
  "$VSI_ROOT/test_pruned.parquet" \
  "$VSI_ROOT/test_debiased.parquet"; do
  if [[ ! -f "$file" ]]; then
...
for split in scannet arkitscenes scannetpp; do
  ...
  if [[ ! -e "$SPATIAL_FEATURES_ROOT/$split/$SPATIAL_FEATURES_SUBDIR" ]]; then
    echo "[ERROR] Missing CUT3R token sidecars: $SPATIAL_FEATURES_ROOT/$split/$SPATIAL_FEATURES_SUBDIR"
    exit 1
  fi
done
...
MODEL_ARGS+=",model_name=$MODEL_NAME,conv_template=$CONV_TEMPLATE,max_frames_num=$MAX_FRAMES_NUM,attn_implementation=$MODEL_ATTN_IMPLEMENTATION"
