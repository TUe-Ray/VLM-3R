...
export RUN_NAME="${RUN_NAME:-oracle_replay_interp_beta_${BETA//./}_single_node_4gpu}"
...
python "$REPO_DIR/scripts/experiments/oracle_replay_interpolation/write_execution_provenance.py" --output-path "$OUTPUT_PATH" --execution-mode single_node_4gpu --world-size 4 --num-processes 4 --accelerate-launch-args "--num_processes 4" --phase start
bash "$REPO_DIR/eval_spatialstack_vsibench.sh"
python "$REPO_DIR/scripts/experiments/oracle_replay_interpolation/validate_evaluation_completion.py" --output-path "$OUTPUT_PATH" --expected-samples "${LIMIT:-5130}" --world-size 4
python "$REPO_DIR/scripts/experiments/oracle_replay_interpolation/write_execution_provenance.py" --output-path "$OUTPUT_PATH" --execution-mode single_node_4gpu --world-size 4 --num-processes 4 --accelerate-launch-args "--num_processes 4" --phase complete
