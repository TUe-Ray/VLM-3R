#!/bin/bash
#SBATCH --job-name=feat_align_frozen_p_geo_100p
#SBATCH --nodes=8
...
# ============================================================
NOTE="Frozen-P_geo spatial readout regularization | VLM-3R init + CE + rank through frozen pretrained P_geo | LoRA(r=128,alpha=256) | Fusion=cross_attention | LambdaSim=0.01"
...
PRETRAINED_P_GEO_RUN_DIR="${PRETRAINED_P_GEO_RUN_DIR:-/leonardo_work/EUHPC_D32_006/Train_Model/VLM3R/feat_align_cut3r_100p_40723512}"
...
SPATIAL_RANK_HEAD_PATH="${SPATIAL_RANK_HEAD_PATH:-$PRETRAINED_P_GEO_RUN_DIR/p_geo.bin}"
P_GEO_EXTRACT_SOURCE="${P_GEO_EXTRACT_SOURCE:-auto}"
FREEZE_SPATIAL_RANK_HEAD="${FREEZE_SPATIAL_RANK_HEAD:-True}"
...
if [[ "$SPATIAL_RANK_LOSS_ENABLE" != "True" ]]; then
    echo "[ERROR] Frozen-P_geo ablation requires SPATIAL_RANK_LOSS_ENABLE=True"
    exit 1
fi

if [[ "$FREEZE_SPATIAL_RANK_HEAD" != "True" ]]; then
    echo "[ERROR] Frozen-P_geo ablation requires FREEZE_SPATIAL_RANK_HEAD=True"
    exit 1
fi

if [[ "$SPATIAL_RANK_HEAD_PATH" == "none" ]]; then
    echo "[ERROR] Frozen-P_geo ablation requires SPATIAL_RANK_HEAD_PATH=/path/to/p_geo.bin"
    exit 1
fi

if [[ ! -f "$SPATIAL_RANK_HEAD_PATH" ]]; then
    echo "[P_GEO] P_geo file not found at $SPATIAL_RANK_HEAD_PATH"
    echo "[P_GEO] Attempting extraction from previous run: $PRETRAINED_P_GEO_RUN_DIR"
    if [[ ! -d "$PRETRAINED_P_GEO_RUN_DIR" ]]; then
        echo "[ERROR] Previous P_geo run dir not found: $PRETRAINED_P_GEO_RUN_DIR"
        exit 1
    fi
    if [[ "$P_GEO_EXTRACT_SOURCE" == "auto" ]]; then
        P_GEO_EXTRACT_SOURCE=""
        if [[ -f "$PRETRAINED_P_GEO_RUN_DIR/non_lora_trainables.bin" ]]; then
            P_GEO_EXTRACT_SOURCE="$PRETRAINED_P_GEO_RUN_DIR/non_lora_trainables.bin"
        else
            mapfile -t P_GEO_CANDIDATES < <(find "$PRETRAINED_P_GEO_RUN_DIR" -maxdepth 2 -type f -name "non_lora_trainables.bin" | sort -V)
    ...
    if [[ -z "$P_GEO_EXTRACT_SOURCE" || ! -f "$P_GEO_EXTRACT_SOURCE" ]]; then
        echo "[ERROR] Could not find non_lora_trainables.bin for P_geo extraction."
...
if [[ ! -f "$SPATIAL_RANK_HEAD_PATH" ]]; then
    echo "[ERROR] P_geo extraction did not create: $SPATIAL_RANK_HEAD_PATH"
    exit 1
fi

echo "[P_GEO] PRETRAINED_P_GEO_RUN_DIR=$PRETRAINED_P_GEO_RUN_DIR"
echo "[P_GEO] SPATIAL_RANK_HEAD_PATH=$SPATIAL_RANK_HEAD_PATH"
...
echo "[SPATIAL_RANK_HEAD] PATH=$SPATIAL_RANK_HEAD_PATH FREEZE=$FREEZE_SPATIAL_RANK_HEAD"

if [[ "$FREEZE_SPATIAL_RANK_HEAD" == "True" && "$SPATIAL_RANK_HEAD_PATH" == "none" ]]; then
    echo "[ERROR] FREEZE_SPATIAL_RANK_HEAD=True requires SPATIAL_RANK_HEAD_PATH=/path/to/p_geo.bin"
    exit 1
fi
...
if [[ "$SPATIAL_RANK_HEAD_PATH" != "none" ]]; then
    TRAINING_ARGS[spatial_rank_head_path]="$SPATIAL_RANK_HEAD_PATH"
fi

if [[ "$FREEZE_SPATIAL_RANK_HEAD" == "True" ]]; then
    TRAINING_ARGS[freeze_spatial_rank_head]="$FREEZE_SPATIAL_RANK_HEAD"
fi
